...
modelpath = "".join([filepath, "k48_5",date_time,"_",filename])

mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
                      save_best_only=True, filepath=filepath+filename)
start_time = time.time()
model.fit(x_train, y_train, epochs=100, batch_size=8,
          verbose=2, validation_split=0.3, callbacks=[es,mcp])
end_time = time.time() - start_time
model.save('./_save/ModelCheckPoint/keras48_5_model_save.h5')

# model2 = load_model('./_save/ModelCheckPoint/keras48_5_model_save.h5')
